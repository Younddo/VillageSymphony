...
public class MemberController {
	...
	public ModelAndView loginProc(@ModelAttribute MemberDTO dto, HttpServletRequest req, HttpServletResponse resp, HttpSession session) {
		...
		if(ulevel==null) {
			...
			String msg="아이디/비밀번호를 다시 한번 확인해주세요";
	...
	public String agreeForm() {
		System.out.println("agreement.do >>>>>> agreeForm");
	...
	public ModelAndView modifyForm(@ModelAttribute MemberDTO dto, HttpSession session) {
		ModelAndView mav = new ModelAndView();
	...
	public ModelAndView memberModify(@ModelAttribute MemberDTO dto, HttpServletRequest request, HttpSession session) {
	...
	public ModelAndView userdelete(@ModelAttribute MemberDTO dto, HttpSession session) {
		ModelAndView mav = new ModelAndView();
		mav.setViewName("member/msgView");
		...
		*/
		...
		if(cnt==0) {
			...
			String msg="회원이 삭제 되었습니다.";		
			
			mav.addObject("msg", msg );
			mav.addObject("url", "/logoutproc.do" );	
