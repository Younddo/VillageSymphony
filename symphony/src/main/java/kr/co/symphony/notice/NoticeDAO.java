	...
	public ArrayList<NoticeDTO> list2(String col, String word, int nowPage, int recordPerPage) {
		...
		try {
			...
			if(word.length()==0) { //검색을 하지 않는 경우
	            sql.append(" SELECT w_num, id, w_title, w_text, w_date, w_views, r ");
	            sql.append(" FROM( SELECT w_num, id, w_title, w_text, w_date, w_views, rownum as r ");
	            sql.append("       FROM ( SELECT w_num, id, w_title, w_text, w_date, w_views ");
	            sql.append("              FROM notice_tb ");
	            sql.append("       WHERE W_CODE = 'n'       ORDER BY w_num DESC ");
	            sql.append("           )");
	            sql.append("     )");
	            ...
	            if(col.equals("w_title")) {
	                ...
	                search += " WHERE w_title LIKE '%" + word + "%' AND W_CODE = 'n' ";
	                search += " OR w_text LIKE '%" + word + "%' AND W_CODE = 'n' ";
