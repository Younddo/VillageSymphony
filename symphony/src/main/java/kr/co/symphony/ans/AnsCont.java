...
public class AnsCont {
	   ...
	   public String check(@RequestBody ArrayList<PayDTO> dtos, HttpSession session) {
			...
			//활용 방법
			//@SuppressWarnings("unchecked") 이거 없애고 노란줄 띄우고 사용해도 됨.
			//ArrayList<CheckDTO> items = (ArrayList<CheckDTO>) session.getAttribute("check");
			//아래 처럼 가지고 와서 쓸 수 있음
			//System.out.println("items"+items.get(0).getC_id());
			
//			PayDTO pay = new PayDTO();
//			pay.setC_id(items.get(0).getC_id());
//			pay.setC_time(items.get(0).getTime());
//			pay.setC_date(items.get(0).getDate());
//			pay.setPrice(items.get(0).getPay());
