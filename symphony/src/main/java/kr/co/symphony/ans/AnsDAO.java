	...
	public ArrayList<RegDTO> resultSel(AnsDTO ans, PageDTO page){
		...
		try {
			...
			if(ans.getQ02ans().equals("E")){
				...
				sql.append("SELECT A.*,B.gender FROM lesson_tb A LEFT JOIN user_tb B on B.ID = A.ID  ");
				sql.append("WHERE Q01ANS = ? AND GENDER IN ('F','M') AND  Q03ANS = ? AND Q04ANS =? ORDER BY DDATE DESC");
				sql.append(")A)B WHERE RM > ((?-1)*?) AND RM <= (?*?)");
				...
				pstmt.setInt(4, page.getNowPage());
				pstmt.setInt(5, page.getAmount());
				pstmt.setInt(6, page.getNowPage());
				pstmt.setInt(7, page.getAmount());
				
			}else{ 
				sql.append("SELECT B.* FROM ");
				sql.append("(SELECT ROWNUM AS RM , A.* FROM (");
				sql.append("SELECT A.*,B.gender FROM lesson_tb A LEFT JOIN user_tb B on B.ID = A.ID  ");
				sql.append("WHERE Q01ANS = ? AND GENDER = ? AND  Q03ANS = ? AND Q04ANS =? ORDER BY DDATE DESC");
				sql.append(")A)B WHERE RM > ((?-1)*?) AND RM <= (?*?)");
				...
				pstmt.setInt(5, page.getNowPage());
				pstmt.setInt(6, page.getAmount());
				pstmt.setInt(7, page.getNowPage());
				pstmt.setInt(8, page.getAmount());
