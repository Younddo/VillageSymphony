...
public ReviewDTO count(ReviewDTO dto) {
	...
	public ArrayList<ReviewDTO> likeList() {
		...
		try {
			...
			sql.append("SELECT ROWNUM RANK ,B.* ");
			sql.append("FROM (SELECT (SELECT COUNT(*) CNT FROM LIKE_TB WHERE W_NUM = A.W_NUM)AS CNT , A.* FROM NOTICE_TB A ");
			sql.append("WHERE W_CODE = 'r' AND A.W_DATE > TRUNC(SYSDATE, 'MM') AND A.W_DATE < LAST_DAY(SYSDATE) ORDER BY CNT DESC, A.W_DATE DESC) B WHERE ROWNUM < 4");
